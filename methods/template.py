...
class MLLTemplate(nn.Module):
    def __init__(self, n_way=16, n_shot=5, n_query=8,
    ...
    def train_loop(self, train_loader, train_dict):
        self.train()
        ...
        for batch in train_loader:
            ...
            selected_values = []
            for i, (key, value) in enumerate(train_dict.items()):
                if sampled_idx[i]:
                    selected_values.append(value)
            
            self.optimizer.zero_grad()
            loss = self.set_forward_loss(x_support, y_support, x_query, y_query, selected_values)
    ...
    def test_loop(self, test_loader, test_dict):
        self.eval()
        num_support = self.n_way * self.n_shot
        ...
        for batch in test_loader:
            ...
            with torch.no_grad():
                y_pred = self.set_forward(x_support, y_support, x_query, selected_values)
    ...
    def load(self, path, epoch=None, load_optimizer=False):
        ...
        self.load_state_dict(tmp['model'], strict=False)
        if load_optimizer:
